...
Now let's install all the dependencies by navigating to the `planetary-config-react` directory and running `npm i`. Then fire up the server with `npm run start` and go to `localhost:8080`
