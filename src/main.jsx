...
class PlanetaryConfigSim extends React.Component {
    ...
    onObserverPlanetAngleUpdate(newAngle) {
        this.stopAnimation();
        let diff = 0;
        const newAng = newAngle;
        const prevObserverPlanetAng = this.state.observerPlanetAngle;
    ...
    onTargetPlanetAngleUpdate(newAngle) {
        ...
        let diff = 0;
        const newAng = newAngle;
        const prevObserverPlanetAng = this.state.targetPlanetAngle;
    ...
    updateMultiplier() {
        const newObserver = Math.pow(this.state.radiusObserverPlanet, -1.5);
        const newTarget = Math.pow(this.state.radiusTargetPlanet, -1.5);
    ...
    onObserverPlanetRadiusChange(e) {
        ...
        if (this.state.radiusObserverPlanet >= this.state.radiusTargetPlanet) {
            ...
            const ratio = (au / this.state.radiusTargetPlanet) * 400;
            this.setState({
    ...
    changeTarget(au) {
        const ratio = (this.state.radiusTargetPlanet / au) * 400;
    ...
    onTargetPlanetRadiusChange(e) {
        ...
        if (this.state.radiusTargetPlanet >= this.state.radiusObserverPlanet) {
            ...
            const ratio = (au / this.state.radiusObserverPlanet) * 400;
            this.setState({
                radiusPixelTarget: forceNumber(ratio),
    ...
    changeObserver(au) {
        const ratio = (this.state.radiusObserverPlanet / au) * 400;
